...
class JianShuList():
    ...
    #提取博客标题和短内容
    def operationBlog(self,url,index):
        ...
        info_div = BeautifulSoup(str(main_top), 'lxml').find('div', class_='info')
        all_p = BeautifulSoup(str(info_div), 'lxml').find_all('p')
        attentionNum = all_p[0].text.strip()
        likeNum = all_p[4].text.strip()

        self.dataManager.insert_Author_item(author,int(attentionNum),int(likeNum),index)
        ...
        for li in all_lis:
            ...
            self.dataManager.insert_Blog_item(name,detail,int(read_num),int(comment_num),int(like_num),index)
